...
export async function GET(req) {
  ...
  // Always expose a reliable userId for linking
  const enhanced = mentors.map(m => {
    const userId = m.userId || m.id; // fallback if old records
    const upcoming = generateUpcomingSlots(m.officeHours || []);
    const taken = new Set(
      bookings
        .filter(b => b.mentorId === userId && b.status !== "cancelled")
        .map(b => b.slotId)
    );
    const available = upcoming.filter(s => !taken.has(s.slotId));
    return { ...m, userId, availableCount: available.length };
  });

  const filtered = domain
    ? enhanced.filter(m => (m.expertise || []).includes(domain))
    : enhanced;
