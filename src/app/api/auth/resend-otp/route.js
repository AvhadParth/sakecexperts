...
export async function POST(req) {
  try {
    ...
    return NextResponse.json({ ok: true, message: "OTP resent", dev: res?.dev ?? false });
  } catch (err) {
    console.error("[SEND-OTP] error:", err && (err.body || err.message || err));
    return NextResponse.json({ ok: false, error: (err && (err.body || err.message)) || "Server error" }, { status: 500 });
  }
}
